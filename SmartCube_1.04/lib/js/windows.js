function windowsjs() {
    ...
    this.game = function () {
        ...
        //初始化
            ...
            //场景与相机
                scene = new THREE.Scene();
                scene.add(anl.craftChecker.group);
                ...
                controlCamera = new anl.controlCamera();
                controlCamera.getObject().position.x = players[0].position.x = control.mapSize;
                controlCamera.getObject().position.y = (players[0].position.y = 250) + 1;
                controlCamera.getObject().position.z = players[0].position.z = control.mapSize;
            //光照
                // var light = new THREE.DirectionalLight(0xffffff,1.0);
            ...
            //启动
                ...
                //玩家定义
                    players[0].things[0] = new anl.things.MAIN_AK47(players[0]);
                    players[0].things[0].action.choosed();
                //鼠标定义
                    anl.moveLogic = this.moveLogic;
                    anl.downLogicL = this.downLogicL;
                    anl.upLogic = this.upLogic;
        ...
        this.downLogicL = function (mx,my) {
            players[0].things[players[0].choosedInt].action.mousedown();
        };
        this.upLogic = function (mx,my) {
            players[0].things[players[0].choosedInt].action.mouseup();
        };
        ...
        this.cycle1 = function () {
            ...
            if (map.time <= 1000) {
                ...
                sun.position.set( players[0].position.x + Math.cos(a)*(control.eyesight - 1), players[0].position.y + Math.sin(a)*(control.eyesight - 1), players[0].position.z );
                moon.position.set( players[0].position.x - Math.cos(a)*(control.eyesight - 1), players[0].position.y - Math.sin(a)*(control.eyesight - 1), players[0].position.z );
                sun.lookAt({x:players[0].position.x,y:players[0].position.y,z:players[0].position.z});
                moon.lookAt({x:players[0].position.x,y:players[0].position.y,z:players[0].position.z});
            } else {
                directionalLight.position.set( 0,-1, 0 ).normalize();
            ...
            sun.lookAt({x:players[0].position.x,y:players[0].position.y,z:players[0].position.z});
            moon.lookAt({x:players[0].position.x,y:players[0].position.y,z:players[0].position.z});
            sky.position.set(players[0].position.x,players[0].position.y,players[0].position.z);
        };
