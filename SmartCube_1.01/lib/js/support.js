function supportjs() {
    ...
    this.getTextures = function (name) {
        if (control.isImage == false) {
            switch (name.split("_")[0]) {
                case "dirt":
                    return new THREE.MeshLambertMaterial({color:0x634822,vertexColors: THREE.VertexColors});
                case "grass":
                    return new THREE.MeshLambertMaterial({color:0x2CD319,vertexColors: THREE.VertexColors});
                default:
                    break;
            }
        } else {
    ...
    this.generateHeight = function (width,height,deepth) {
        ...
        for (var z = 0; z < rData.length; z++) {
            for (var x = 0; x < rData[z].length; x++) {
                for (var y = rData[z].length - 2; y >= 0; y--) {
                    try {
    ...
    this.startWalkW = function (num) {
        if (players[num].isWalking.W == false) {
            ...
            players[num].cycles.W[players[num].cycles.W.length] = window.setInterval(function () {
                if (players[num].speedXZ < 0.1) {
                    players[num].speedXZ += 0.0005;
                }
                if (Math.sin(players[num].face) > 0) {
                    if (anl.getType(map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.3 - players[num].speedXZ * Math.sin(players[num].face))][0|(players[num].ry + 0.000001)].name) != "hard") {
                        controlCamera.getObject().position.x = players[num].rx -= players[num].speedXZ * Math.sin(players[num].face);
                    }
                } else if (Math.sin(players[num].face) < 0) {
                    if (anl.getType(map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.7 - players[num].speedXZ * Math.sin(players[num].face))][0|(players[num].ry + 0.000001)].name) != "hard") {
                        controlCamera.getObject().position.x = players[num].rx -= players[num].speedXZ * Math.sin(players[num].face);
                    }
                }
                if (Math.cos(players[num].face) > 0) {
                    if (anl.getType(map.data[0|(players[num].rz + 0.3 - players[num].speedXZ * Math.cos(players[num].face))][0|(players[num].rx + 0.5)][0|(players[num].ry + 0.000001)].name) != "hard") {
                        controlCamera.getObject().position.z = players[num].rz -= players[num].speedXZ * Math.cos(players[num].face);
                    }
                } else if (Math.cos(players[num].face) < 0) {
                    if (anl.getType(map.data[0|(players[num].rz + 0.7 - players[num].speedXZ * Math.cos(players[num].face))][0|(players[num].rx + 0.5)][0|(players[num].ry + 0.000001)].name) != "hard") {
                        controlCamera.getObject().position.z = players[num].rz -= players[num].speedXZ * Math.cos(players[num].face);
                    }
                }
            },players[num].speed);
    ...
    this.checkPlayer = function (num) {
        if (0|(players[num].ry + players[num].speedY/20) < 0 || 0|(players[num].ry + 1) >= map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.5)].length) {
            ...
            if (anl.getType(map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.5)][0|(players[num].ry -0.001 + players[num].speedY*control.checkSpeed/1000)].name) != "hard") {
                players[num].ry += players[num].speedY*control.checkSpeed/1000;
                if (players[num].speedY > - 40) {
                    players[num].speedY -= control.graxity*control.checkSpeed/1000;
                }
    ...
    this.jump = function (num) {
        if (players[num].isWalking.Space == false) {
            ...
            players[num].cycles.Space[players[num].cycles.Space.length] = window.setInterval(function () {
                if (anl.getType(map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.5)][0|(players[num].ry + 2.6)].name) != "hard" && players[num].speedY <= 0) {
                    if (anl.getType(map.data[0|(players[num].rz + 0.5)][0|(players[num].rx + 0.5)][0|(players[num].ry -0.2)].name) == "hard"){
